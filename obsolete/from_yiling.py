...
def calculate_surprise2():
    ...
    ThreeBreakthroughPaper_References_Citations_Path = (
        os.path.join(git_data_dir, "", "ThreeBreakthroughPaper_References_Citations.txt"))  # future abstract
    RandomPaper_References_Citations_Path = (
        os.path.join(git_data_dir, "", "YEARRandomPaper_References_Citations.txt"))  # future abstract
    ...
    papers_to_test = list(InformationTheoryCases.keys()) + [p for p in RandomPaper_C if
                                                            len(RandomPaper_C[p]) >= max_num_cit]
    ...
    for paper_to_test in papers_to_test:
        ...
        for citation in focal_paper_refs:
            ...
            if citation in PaperAbstract:
                ...
                for w in PaperAbstract[citation]:
                    content_0_words += [w] * len(PaperAbstract[citation][w])
        ...
        for citation in focal_paper_cits:
            ...
            if citation in PaperAbstract:
                ...
                for w in PaperAbstract[citation]:
                    content_2_words += [w] * len(PaperAbstract[citation][w])
                cit_found = cit_found + 1
            else:
                print("can't find " + citation)
            # if cit_found >= max_num_cit:
        ...
        Novelty_of_abs_1_given_abs_0 = calculate_surprise_between_two_word_lists(content_1_words,
                                                                                 content_0_words)

        # how surprising is content_2 given content_1?
        Novelty_of_abs_2_given_abs_1 = calculate_surprise_between_two_word_lists(content_2_words,
                                                                                 content_1_words)
        # Novelty_of_abs_2_given_abs_0 = calculate_surprise_between_two_word_lists(content_0_words,
        #                                                                         content_2_words)

        Novelty_of_abs_2_given_abs_0 = calculate_surprise_between_two_word_lists(
            content_2_line, content_0_line)
        ...
        Transience_of_abs_1 = calculate_surprise_between_two_word_lists(
        ...
        results[paper_to_test] = [type, Resonance_of_abs_1, Impact_of_abs_1, Transience_of_abs_1]

    with open(output_path, "w") as file:
        file.write(f"Paper,Type,Resonance,Impact,Transience\n")
        for key, value in results.items():
            file.write(f"{key},{value[0]},{value[1]},{value[2]},{value[3]}\n")


def calculate_surprise_between_two_word_lists(testing_words, training_words, ):
    # all_words = list(set(training_words + testing_words))
    all_words = list(set(training_words).intersection(set(testing_words)))
